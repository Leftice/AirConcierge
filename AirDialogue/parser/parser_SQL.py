...
if args.syn:
    if not os.path.exists('./synthesized_sql/'):
        os.makedirs('./synthesized_sql/')
    sql_path = './synthesized_sql/'
    data_path = '../data/synthesized/'
elif args.air:
    if not os.path.exists('./airdialogue_sql/'):
        os.makedirs('./airdialogue_sql/')
...
if args.dev:
    sql_fp = open(sql_path + 'dev.sql.txt', 'w')
    log_fp = open(sql_path + 'dev.data.log', 'w')
    data_fp = open(data_path + 'tokenized/dev/dev.eval.data', "r")
    kb_fp = open(data_path + 'tokenized/dev/dev.eval.kb', "r")
elif args.train:
    sql_fp = open(sql_path + 'train.sql.txt', 'w')
    log_fp = open(sql_path + 'train.data.log', 'w')
    ...
    sql_fp = open(sql_path + 'selfplay_eval.sql.txt', 'w')
    log_fp = open(sql_path + 'selfplay_eval.data.log', 'w')
    data_fp = open(data_path + 'tokenized/selfplay_eval/dev.selfplay.eval.data', "r")
    kb_fp = open(data_path + 'tokenized/selfplay_eval/dev.selfplay.eval.kb', "r")
else:
...
for mydic in intent_list:
    log_fp.write('*'*50 +'\n')
    log_fp.write('{:26} : {} \n'.format(intent_name_list[i], len(mydic.keys())))
    i +=1
    for key, value in mydic.items() :
        log_fp.write('    >>  {:26} : {:<6} prob : {:>4.1f} % \n'.format(key,  value, 100. * value /sum(mydic.values())))
log_fp.write('*'*50 +'\n')
print('Finish generating SQL !')