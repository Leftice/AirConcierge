...
class Checkpoint(object):
    ...
    def __init__(self, model, optimizer, epoch, step, input_vocab=None, output_vocab=None, path=None):
        self.model = model
    ...
    def save(self, experiment_dir):
        ...
        self._path = os.path.join(experiment_dir, date_time)
        path = self._path
        ...
        torch.save(state_other, os.path.join(path, self.TRAINER_STATE_NAME))
        torch.save(state_model, os.path.join(path, self.MODEL_NAME))
    ...
    def load(cls, path):
        ...
        return Checkpoint(model=model['net'],
                          optimizer=resume_checkpoint['optimizer'],
