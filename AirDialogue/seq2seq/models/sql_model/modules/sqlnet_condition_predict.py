...
class SQLNetCondPredictor(nn.Module):
    ...
    def forward(self, x_emb_var, x_len, col_inp_var, col_name_len,
        ...
        e_num_col, col_num = col_name_encode(col_inp_var, col_name_len, col_len, self.cond_num_name_enc) # run lstm(sort, pad) + pad with zero
        print('col_inp_var : ', col_inp_var.size())
        print('col_name_len : ', col_name_len)
        print('col_len : ', col_len)
        print('e_num_col : ', e_num_col.size())
        print('col_num (col_len) : ', col_num)
        num_col_att_val = self.cond_num_col_att(e_num_col)
        print('num_col_att_val : ', num_col_att_val.size())
        num_col_att_val = num_col_att_val.squeeze()
        print('num_col_att_val : ', num_col_att_val.size())
        for idx, num in enumerate(col_num):
            if num < max(col_num):
                num_col_att_val[idx, num:] = -100
        num_col_att = self.softmax(num_col_att_val)
        temp = e_num_col * num_col_att.unsqueeze(2)
        print('e_num_col * num_col_att : ', temp.size())
        K_num_col = (e_num_col * num_col_att.unsqueeze(2)).sum(1)
        print('K_num_col : ', K_num_col.size())
        
        cond_num_h1 = self.cond_num_col2hid1(K_num_col)
        print('cond_num_h1 : ', cond_num_h1.size())
        cond_num_h1 = cond_num_h1.view(B, 4, self.N_h/2).transpose(0, 1).contiguous() # bi-lstm d=2 -> need 4 init hidden state
        print('cond_num_h1 2: ', cond_num_h1.size())
        ...
        print('h_num_enc : ', h_num_enc.size())
        num_att_val = self.cond_num_att(h_num_enc).squeeze()
        print('num_att_val : ', num_att_val.size())
        ...
        cond_num_score = self.cond_num_out(K_cond_num) # nn(100, 100) tahn() nn(100, 5) k = 0 ~ 4
        print('cond_num_score : ', cond_num_score.size())
        ...
        if self.use_ca:
            temp = self.cond_col_att(h_col_enc)
            print('h_col_enc : ', h_col_enc.size())
            print('self.cond_col_att(h_col_enc) : ', temp.size())
            ...
            temp = h_col_enc.unsqueeze(1) * col_att.unsqueeze(3)
            K_cond_col = (temp).sum(2)
            print('col_att : ', col_att.size())
            print('h_col_enc.unsqueeze(1) : ', h_col_enc.unsqueeze(1).size())
            print('col_att.unsqueeze(3) : ', col_att.unsqueeze(3).size())
            print('h_col_enc.unsqueeze(1) * col_att.unsqueeze(3) : ', temp.size())
            print('K_cond_col : ', K_cond_col.size())
        ...
        if gt_cond is None:
            ...
            print('chosen_col_gt : ', chosen_col_gt)
        else:
            print(aeoioiqe)
        ...
        print('*'*100)
        print('op')
        print('e_cond_col : ', e_cond_col.size())
        print('chosen_col_gt : ', chosen_col_gt)
        for b in range(B):
        ...
        print('col_emb : ', len(col_emb))
        col_emb = torch.stack(col_emb)
        print('col_emb stack : ', col_emb.size())

        h_op_enc, _ = run_lstm(self.cond_op_lstm, x_emb_var, x_len)
        print('h_op_enc : ', h_op_enc.size())
        if self.use_ca:
            print('self.cond_op_att(h_op_enc).unsqueeze(1) : ', self.cond_op_att(h_op_enc).unsqueeze(1).size())
            print('col_emb.unsqueeze(3) : ', col_emb.unsqueeze(3).size())
            ...
            print('op_att_val : ', op_att_val.size())
            print('op_att : ', op_att.size())
            temp_op = h_op_enc.unsqueeze(1) * op_att.unsqueeze(3)
            print('temp : ', temp_op.size())
            K_cond_op = (temp_op).sum(2)
            print('K_cond_op : ', K_cond_op.size())
        else:
        ...
        print('cond_op_score : ', cond_op_score.size())
        ...
        if gt_where is not None:
            ...
            while t < 50:
                ...
                cur_cond_str_score = self.cond_str_out(
                        ...
                        + self.cond_str_out_col(col_ext)).squeeze()
                for b, num in enumerate(x_len):
                    if num < max_x_len: # max_x_len is not fixed
