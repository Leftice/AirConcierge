...
def process_entry_self_play_eval(inputs, vocab_table, t1_id, t2_id, table_data):
    """Pre-process procedure for the supervised iterator."""
    intent, action, kb, kb_true_answer, SQL_YN, sql_data = inputs[0], inputs[1], inputs[2], inputs[3], inputs[4], inputs[5]
    ...
    return intent, size_intent, action, kb, has_reservation, col_seq, col_num, ans_seq, truth_seq, kb_true_answer, SQL_YN
...
def loader(batch_size, toy, max_len=None, need_shuffle=False, mask=False, only_f=False, dev=False, n_sample=-1, small_n=30, args=None):
    ...
    if toy : 
        data_file = pre_data_path + 'tokenized/sub_air/train_sub.data'
        kb_file = pre_data_path + 'tokenized/sub_air/train_sub.kb'
        table_path = pre_data_path + 'SQL/train/train_tok.tables.jsonl'
        sql_path = pre_data_path + 'SQL/train/train_tok.jsonl'
        stk_path = pre_data_path + 'SQL/train/State_Tracking.txt'
        filter_kb = pre_data_path + 'SQL/train/filtered_kb'
        print('Using toy !')
    elif dev:
        data_file = pre_data_path + 'tokenized/dev/dev.eval.data'
        kb_file = pre_data_path + 'tokenized/dev/dev.eval.kb'
        table_path = pre_data_path + 'SQL/dev/dev_tok.tables.jsonl'
        sql_path = pre_data_path + 'SQL/dev/dev_tok.jsonl'
        stk_path = pre_data_path + 'SQL/dev/State_Tracking.txt'
        filter_kb = pre_data_path + 'SQL/dev/filtered_kb'
    else:
        data_file = pre_data_path + 'tokenized/train/train.data'
        kb_file = pre_data_path + 'tokenized/train/train.kb'
        table_path = pre_data_path + 'SQL/train/train_tok.tables.jsonl'
        sql_path = pre_data_path + 'SQL/train/train_tok.jsonl'
        stk_path = pre_data_path + 'SQL/train/State_Tracking.txt'
        filter_kb = pre_data_path + 'SQL/train/filtered_kb'
...
def Inference_loader(batch_size, toy, max_len=None, need_shuffle=False, mask=False, only_f=False, dev=False, n_sample=-1, small_n=30, args=None):
    global eod_id
    global smallkb_n
    smallkb_n = small_n
    # print('Kb_n : ', smallkb_n)
    ...
    if toy : 
        ...
        kb_file = pre_data_path + 'tokenized/sub_air/toy_dev.infer.kb'
        table_path = pre_data_path + 'SQL/dev/dev_tok.tables.jsonl'
        print('Using toy !')
    elif dev:
...
def SelfPlayEval_loader(batch_size, toy, max_len=None, need_shuffle=False, mask=False, only_f=False, dev=False, n_sample=-1, small_n=30, args=None):
    global eod_id
    global smallkb_n
    smallkb_n = small_n
    # print('Kb_n : ', smallkb_n)
