...
if args.dev:
    kb_file =  'data/airdialogue/tokenized/dev.kb'
    data_file = 'data/airdialogue/tokenized/dev.data'

    query_file = 'SQL/dev_sql/dev_predict_query'
    query2_file = 'SQL/dev_sql/dev_simple' 
    true_query_file = 'SQL/dev_sql/dev_gt_query'
    gate_file = 'SQL/dev_sql/dev_gate'

    if not os.path.exists('SQL/dev_sql/simulate_DB/'):
        os.makedirs('SQL/dev_sql/simulate_DB/')
    small_fp = open('SQL/dev_sql/simulate_DB/small_db.kb', 'w')
    r_fp = open('SQL/dev_sql/simulate_DB/record', 'w')
    rf_fp = open('SQL/dev_sql/simulate_DB/filtered_kb', 'w')
elif args.train:
    kb_file =  'data/airdialogue/tokenized/train_new.kb'
    data_file = 'data/airdialogue/tokenized/train.data'

    query_file = 'train_sql/train_predict_query'
    query2_file = 'train_sql/train_simple' 
    true_query_file = 'train_sql/train_gt_query'
    gate_file = 'train_sql/train_gate'
    
    if not os.path.exists('train_sql/simulate_DB/'):
        os.makedirs('train_sql/simulate_DB/')
    small_fp = open('train_sql/simulate_DB/small_db.kb', 'w')
    r_fp = open('train_sql/simulate_DB/record', 'w')
    rf_fp = open('train_sql/simulate_DB/filtered_kb', 'w')
else:
...
def simulate_DB(kb, true_query, truth_gate, query, query2, gate, condiction_num, sort_indices, sort_sent):
    ...
    for i in range(len(kb)):
        ...
        if i > 0 and i < 5:
            ...
            print('air : ', airport_list[true_query_i[0]], ' ', airport_list[true_query_i[1]] ,'month : ', month_list[true_query_i[2]], ' ', month_list[true_query_i[3]])
        ...
        for entry in range(len(kb[i])):
            ...
            if correct == 1:
                ...
                filtered_kb[entry] = 1
            if i > 0 and i < 5:
                print(filtered_kb[entry], ' kb[i][entry] : ', kb[i][entry])
...
simulate_DB(kb_sents, sort_true_query, sort_truth_gate, sort_query, sort_query2, sort_gate, 6, list(range(len(kb_sents))), sents)
