...
if args.self_play_dev:
    ...
    predict_query_file = 'Self_Play_Eval/SQL/predicted_.txt'
    # true_query_file = 'Self_Play_Eval/SQL/ground_truth_.txt'
    if not os.path.exists('Self_Play_Eval/SQL/dev_sql/simulate_DB/'):
        os.makedirs('Self_Play_Eval/SQL/dev_sql/simulate_DB/')
    filtered_fp = open('Self_Play_Eval/SQL/dev_sql/simulate_DB/filtered_index.kb', 'w')
    filtered_fp2 = open('Self_Play_Eval/SQL/dev_sql/simulate_DB/filtered.kb', 'w')
elif args.infer_dev:
    kb_file =  'data/synthesized/tokenized/dev.infer.kb'
    predict_query_file = 'Inference_Bleu/SQL/predicted_.txt'
    if not os.path.exists('Inference_Bleu/SQL/dev_sql/simulate_DB/'):
        os.makedirs('Inference_Bleu/SQL/dev_sql/simulate_DB/')
    filtered_fp = open('Inference_Bleu/SQL/dev_sql/simulate_DB/filtered_index.kb', 'w')
    filtered_fp2 = open('Inference_Bleu/SQL/dev_sql/simulate_DB/filtered.kb', 'w')
else:
...
def simulate_DB(kb, query, gate, condiction_num, reservations):
    ...
    for i in range(len(kb)):
        ...
        if i == 0:
            print('*'*100)
            print('query : ', query[i], 'gate : ', gate[i])
            print('air : ', airport_list[query_i[0]], ' ', airport_list[query_i[1]] ,'month : ', month_list[query_i[2]], ' ', month_list[query_i[3]])
...
print('Size of query : ', len(predict_query))
print('Size of each query : ', len(predict_query[0]), predict_query[0])
print('Size of each gate  : ', predict_gate[0])
