...
class SupervisedInferencePrior(object):
    ...
    def _test_epoches_t1t2(self, dataloader, model, start_step, save_dir='runs/exp', args=None):
        ...
        if not os.path.exists('Inference_Bleu/SQL/'):
            os.makedirs('Inference_Bleu/SQL/')
        gt_fp = None
        p_fp = open('Inference_Bleu/SQL/predicted_.txt', 'w')
        sql_fp = open('Inference_Bleu/SQL/predicted_SQL.txt', 'w')
        gate_fp = open('Inference_Bleu/SQL/prior_gate.txt', 'w')
