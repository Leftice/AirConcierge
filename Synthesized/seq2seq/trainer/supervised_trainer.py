...
class SupervisedTrainer(object):
    ...
    def _test_epoches(self, dataloader, model, n_epochs, start_epoch, start_step, dev_data=None, teacher_forcing_ratio=0, clip=5.0, save_dir='runs/exp', args=None):
        ...
        # eval sql acc
        if args.dev:
            if not os.path.exists('SQL/dev_sql/'):
                os.makedirs('SQL/dev_sql/')
            all_fp = open('SQL/dev_sql/dev_predict_gt', 'w')
            predict_fp = open('SQL/dev_sql/dev_predict_query', 'w')
            gt_fp = open('SQL/dev_sql/dev_gt_query', 'w')
            gate_fp = open('SQL/dev_sql/dev_gate', 'w')
            simple_fp = open('SQL/dev_sql/dev_simple', 'w')
            p_simple_fp = open('SQL/dev_sql/dev_simple_predict', 'w')
            g_simple_fp = open('SQL/dev_sql/dev_simple_ground', 'w')
            ...
            if not os.path.exists('SQL/train_sql/'):
                os.makedirs('SQL/train_sql/')
            all_fp = open('SQL/train_sql/train_predict_gt', 'w')
            predict_fp = open('SQL/train_sql/train_predict_query', 'w')
            gt_fp = open('SQL/train_sql/train_gt_query', 'w')
            gate_fp = open('SQL/train_sql/train_gate', 'w')
            simple_fp = open('SQL/train_sql/train_simple', 'w')
