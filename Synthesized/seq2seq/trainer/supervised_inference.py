...
class SupervisedInference(object):
    ...
    def _test_epoches_t1(self, dataloader, model, start_step, save_dir='runs/exp', args=None):
        ...
        if not os.path.exists('Inference_Bleu/t1/'):
            os.makedirs('Inference_Bleu/t1/')
        t1_fp = open('Inference_Bleu/t1/t1_output.txt', 'w')
        with torch.no_grad():
            train_loss = 0
            total = 0
    ...
    def _test_epoches_t2(self, dataloader, model, start_step, save_dir='runs/exp', args=None):
        ...
        model.eval()
        total_step = start_step
        if not os.path.exists('Inference_Bleu/t2/'):
            os.makedirs('Inference_Bleu/t2/')
        t2_fp = open('Inference_Bleu/t2/t2_output.txt', 'w')
        with torch.no_grad():
            train_loss = 0
            total = 0
