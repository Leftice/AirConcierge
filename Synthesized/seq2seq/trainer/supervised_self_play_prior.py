...
class SupervisedSelfPlayPrior(object):
    ...
    def _test_epoches_t1t2_self_play_eval(self, dataloader, model, start_step, save_dir='runs/exp', args=None):
        ...
        if not os.path.exists('Self_Play_Eval/Prior/'):
            os.makedirs('Self_Play_Eval/Prior/')
        t_fp = open('Self_Play_Eval/Prior/self_output.data', 'w')

        if not os.path.exists('Self_Play_Eval/SQL/'):
            os.makedirs('Self_Play_Eval/SQL/')
        gt_fp = open('Self_Play_Eval/SQL/ground_truth_.txt', 'w')
        p_fp = open('Self_Play_Eval/SQL/predicted_.txt', 'w')
        sql_fp = open('Self_Play_Eval/SQL/predicted_SQL.txt', 'w')
        gate_fp = open('Self_Play_Eval/SQL/prior_gate.txt', 'w')
        fp = [gt_fp, p_fp, sql_fp]

        with torch.no_grad():
            total = 0
            for batch_idx, (intent, size_intent, action, kb, has_reservation, col_seq, truth_seq, SQL_YN, kb_true_answer) in enumerate(dataloader):
                
                intent, size_intent, action, kb, has_reservation, col_seq, SQL_YN, kb_true_answer = intent.cuda(), size_intent.cuda(), action.cuda(), kb.cuda(), has_reservation.cuda(), col_seq.cuda(), SQL_YN.cuda(), kb_true_answer.cuda()
